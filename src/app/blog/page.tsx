    ...
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
          ...
          <Link
            href="/"
            className="inline-flex items-center px-4 py-2 bg-black text-white font-medium rounded-lg hover:bg-neutral-900 focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 transition-colors duration-200"
          >
            ← Return to Home
          </Link>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="overflow-x-auto lg:overflow-x-visible">
                <ul className="flex lg:flex-col gap-3 min-w-min">
                  {postsWithTitles.map((post) => {
                    ...
                    const isSelected = post.filename === selectedPostFilename;
                    return (
                      <li key={post.filename} className="min-w-[250px] lg:min-w-0">
                        <Link
                          href={`/blog?post=${post.filename}`}
                          className={`block p-2 rounded transition-colors duration-200 hover:bg-gray-100 ${
                            isSelected
                        ...
                        >
                          ...
                          <div className="text-sm font-medium mt-1">{post.title}</div>
                        </Link>
                      </li>
                    );
                  })}
          ...
          {/* Main Content */}
          <article className="flex-1 prose prose-lg max-w-none prose-a:text-black hover:prose-a:opacity-80">
            <div
